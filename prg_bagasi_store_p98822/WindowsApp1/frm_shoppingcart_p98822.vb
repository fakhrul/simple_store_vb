...
Public Class frm_shoppingcart_p98822
    ...
    Public Event ClearCart(ByVal sender As Object, ByVal e As EventArgs)

    Protected Overridable Sub OnClearCart()
        RaiseEvent ClearCart(Me, New EventArgs())
    End Sub
    ...
    Private Sub btn_CheckOut_Click(sender As Object, e As EventArgs) Handles btn_CheckOut.Click
        ...
        If confirmation = MsgBoxResult.Yes Then
            Try
                Dim orderId As Integer = InsertOrder()
    ...
    Private Function InsertOrder() As Integer
    ...
    Private Sub InsertOrderItem(orderId As Integer)
        For Each productId In shoppingCart
            Dim mysql As String = "insert into tbl_order_product_p98822 (FLD_order_id, FLD_product_id, FLD_order_product_quantity) values (" &
            orderId & "," &
            productId & "," &
            "1)"
            Using connection As OleDbConnection = New OleDbConnection(myconnection)
